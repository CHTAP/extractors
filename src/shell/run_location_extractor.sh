...
do  
    ...
    ssh -t jdunnmon@${NODES[NODE_INDEX]} <<< "export SNORKELDB=postgresql://jdunnmon:123@localhost:5432/${DBNAME}; export CUDA_VISIBLE_DEVICES=0; cd /dfs/scratch1/jdunnmon/repos/extractors/src/shell; source activate snorkel; echo 'Running location candidate extraction...'; python extract_location_candidates.py -f ${FOLDERS[$iter]}; echo 'Evaluating location extractor for node ${NODES[NODE_INDEX]} with database ${DBNAME} ...'; python evaluate_location_extractor.py -f ${FOLDERS[$iter]}; bash -l" &
done

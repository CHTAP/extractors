...
do  
    ...
    ssh -t jdunnmon@${NODES[$iter]} <<< "export SNORKELDB=postgresql://jdunnmon:123@localhost:5432/${DBNAME}; export CUDA_VISIBLE_DEVICES=0; cd /dfs/scratch1/jdunnmon/repos/extractors/src/shell; source activate snorkel; echo 'Running location candidate extraction...'; python extract_location_candidates.py -f ${FOLDERS[$iter]}; echo 'Running price candidate extraction...'; python extract_price_candidates.py -f ${FOLDERS[$iter]}; bash -l" &
done

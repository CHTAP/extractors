...
do  
    ...
    ssh -t jdunnmon@${NODES[NODE_INDEX]} <<< "export SNORKELDB=postgresql:///localhost:5432/${DBNAME}; export CUDA_VISIBLE_DEVICES=0; cd /dfs/scratch1/jdunnmon/repos/extractors/src/shell; source activate snorkel; echo 'Running raw content extractor for node ${NODES[NODE_INDEX]} with database ${DBNAME} ...'; python create_raw_content_jsonls.py -f ${FOLDERS[$iter]}; bash -l" &
done
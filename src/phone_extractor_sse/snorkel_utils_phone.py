...
def get_gold_phone_label_from_meta(session, candidate_class, target, split, annotator='gold', gold_dict=None):
    ...
    for i , c in enumerate(candidates):
        ...
        #print("extracted cand", extracted_cand)
        
        if gold_dict is None:
            try:
                gold_value = get_extraction_from_candidate(c,target,extractions_field='extractions')
            except:
                print('Gold label not found!')
               # import pdb; pdb.set_trace()
                continue
...
#         gold_dict = gold_dict 
#         gold_value = gold_dict[url]
#         if url in list(gold_dict.keys()):
            
        if target == 'phone': 
            if extracted_cand in gold_value:
                label = 1
            else:
                label = -1
            ...
            if existing_label is None:
                session.add(GoldLabel(candidate=c, key=ak, value=label))                        
                labels+=1
            print("label:", label,"gold_value: ",gold_value, "extracted_cand: ",extracted_cand)
    pb.close()
    print("AnnotatorLabels created: %s" % (labels,))
...
#                 continue
        ...
        ## check extracted candidate in gold_value_list
#           if extracted_cand in gold_value:
#                 label = 1
#             else:
#                 label = -1
...
#     candidates = session.query(candidate_class).filter(candidate_class.split == split).all()
#     for i, c in enumerate(candidates):
#         id_=c.get_parent().document.name
#         label = 